...
export function Terminal({
  ...
  const inViewObserved = useInView(containerRef);
  // Fall back to true after 800 ms so the animation starts even when
  // IntersectionObserver doesn't fire (e.g. inside overflow:hidden parents).
  const [inViewFallback, setInViewFallback] = useState(false);
  useEffect(() => {
    const t = setTimeout(() => setInViewFallback(true), 800);
    return () => clearTimeout(t);
  }, []);
  const inView = inViewObserved || inViewFallback;
  ...
  /* Prompt matches the reference: cyan username + muted :~$ */
  const prompt = (
    <span>
      <span style={{ color: "#38bdf8", fontWeight: 600 }}>{username}</span>
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: "100%", fontFamily: "Geist Mono, monospace", fontSize: 13 }}
      className={className}
    >
      <div style={{
        ...
        background: "#000000",
        boxShadow: "0 32px 64px rgba(0,0,0,0.65), 0 8px 24px rgba(0,0,0,0.4)",
      }}>
        {/* Title bar — traffic lights left, title centered, spacer right */}
        <div style={{
          display: "flex", alignItems: "center",
          background: "#111111", padding: "11px 16px",
          borderBottom: "1px solid rgba(255,255,255,0.06)",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, width: 52 }}>
          ...
          <div style={{ flex: 1, textAlign: "center" }}>
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", letterSpacing: "-0.01em", fontFamily: "Geist Mono, monospace" }}>
              {username} — bash
            </span>
          </div>
          <div style={{ width: 52 }} />
        ...
        <div
          ref={contentRef}
          style={{
            height: 340, overflowY: "auto", padding: "18px 20px",
            fontFamily: "Geist Mono, monospace", fontSize: 13,
            lineHeight: 1.75, scrollbarWidth: "none",
            color: "#cdd6f4",
            background: "#000000",
          }}
        >
          {lines.map((line, i) => (
            <div key={i} style={{ whiteSpace: "pre-wrap" }}>
              {line.type === "command" ? (
                ...
                <span style={{ color: "#a6adc8" }}>{line.content}</span>
              )}
            </div>
          ))}
          ...
          {phase === "typing" && (
            <div style={{ whiteSpace: "pre-wrap" }}>
              ...
              <span style={{
                ...
                background: "#cdd6f4", verticalAlign: "middle", marginLeft: 1,
              }} />
            </div>
          )}
