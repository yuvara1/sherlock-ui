...
const LANG_HUE: Record<string, number> = {
  "Java": 200, "Node.js": 120, "Go": 195, "Python": 47, "Scala": 350,
};
function langTokens(lang: string) {
  const hue = LANG_HUE[lang] ?? 260;
  return { color: `hsl(${hue},65%,60%)`, bg: `hsl(${hue},55%,10%)`, border: `hsl(${hue},35%,18%)` };
}
...
function LangCell({ data }: ICellRendererParams<Service>) {
  if (!data) return null;
  const t = langTokens(data.lang);
  return (
    <Cell>
      <span style={{ display: "inline-flex", alignItems: "center", fontSize: 10, fontWeight: 500, padding: "1px 6px", borderRadius: 4, color: t.color, background: t.bg, border: `1px solid ${t.border}`, whiteSpace: "nowrap", fontFamily: "Geist, sans-serif", letterSpacing: "0.01em" }}>
        {data.lang}
      </span>
    </Cell>
  );
  ...
  return (
    <Drawer open={open} onOpenChange={o => { if (!o) onClose(); }} direction="right">
      <DrawerContent
        ...
        style={{ background: "var(--bg)", borderColor: "var(--border)", maxWidth: "90vw" }}
      >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Description */}
          <p style={{ fontSize: 12, lineHeight: 1.7, color: "var(--text-3)", margin: 0 }}>{service.desc}</p>
          ...
          {/* Utilization bars */}
          <div style={{ border: "1px solid var(--border)", borderRadius: 9, background: "var(--bg-2)", overflow: "hidden" }}>
          ...
          {/* Active incidents */}
          {service.incidents.length > 0 && (
            <div style={{ border: `1px solid ${t.border}`, borderRadius: 9, background: t.bg, overflow: "hidden" }}>
          ...
          <div style={{ border: "1px solid var(--border)", borderRadius: 9, background: "var(--bg-2)", overflow: "hidden" }}>
            ...
            {service.deps.map(d => (
              <div key={d} style={{ padding: "9px 14px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", transition: "background 0.1s" }}
